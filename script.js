...
function renderBreakdown(scope) {
  ...
  sorted.forEach((r) => {
    const color = r.acc >= 80 ? "var(--lime)" : r.acc >= 55 ? "var(--amber)" : "var(--coral)";
    const row = document.createElement("div");
    row.className = "bar-row";
    row.innerHTML = `
      <div class="bar-label">${OPS[r.k].name}<small>${r.correct}/${r.total}</small></div>
      <div class="bar-track">
        <div class="bar-fill" style="background:${color}">${r.acc}%</div>
        <span class="bar-time">${r.avg.toFixed(1)}s</span>
      </div>`;
    chart.appendChild(row);
    requestAnimationFrame(() => {
      row.querySelector(".bar-fill").style.width = Math.max(r.acc, 14) + "%";
    });
  });
}
